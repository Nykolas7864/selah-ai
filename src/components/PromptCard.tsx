 ...
 * Highlighted names open a glossary side panel (desktop) or bottom sheet (mobile).
 */
export default function PromptCard({ prompt, compact = true }: PromptCardProps) {
  const [expanded, setExpanded] = useState(!compact);
  const [copied, setCopied] = useState(false);
  const [selectedFigure, setSelectedFigure] = useState<GlossaryEntry | null>(null);
  ...
  const handleNameClick = (name: string) => {
    ...
    if (entry) setSelectedFigure(entry);
  };
      ...
      >
        <div className="p-6">
          ...
          <h2 className="font-serif text-lg leading-relaxed text-olive-900 mb-2">
            <HighlightedText
              text={prompt.mainQuestion}
              onNameClick={handleNameClick}
              className="font-serif text-lg leading-relaxed text-olive-900"
            />
          </h2>
          ...
          {expanded && (
            <div className="mt-6 space-y-6 animate-fadeIn">
              ...
              <HighlightedText
                text={prompt.context}
                onNameClick={handleNameClick}
                className="text-olive-700 font-sans leading-relaxed"
              />
                  ...
                  {prompt.subQuestions.map((sq, i) => (
                    ...
                    >
                      <HighlightedText
                        text={sq.question}
                        onNameClick={handleNameClick}
                        className="font-sans text-olive-800 leading-relaxed"
                      />
                      {sq.verseReference && (
